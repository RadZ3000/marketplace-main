...
function clearAllAttributeKeys(router: NextRouter) {
  let query = router.query
  ...
  Object.keys(query).find((key) => {
    ...
    if (key === 'price[lte]') delete query[key]
    if (key === 'price[gte]') delete query[key]
  })
...
export { toggleOffItem, toggleOnItem, updateItem, toggleOnAttributeKey, clearAllAttributeKeys }
