        ...
        <tbody>
          {transfersFlat.map((transfer, index, arr) => {
            const {
              ...
              price,
              collectionName,
            } = processTransfer(transfer, account?.address, chainId)
            return (
              <tr
              ...
              >
                ...
                {/* PRICE */}
                <td className="reservoir-body whitespace-nowrap px-6 py-4 dark:text-white">
                  <FormatPrice amount={price} organization={organization} />
                </td>

                {/* FROM */}
                <td className="reservoir-body whitespace-nowrap px-6 py-4 dark:text-white">
                  {from && (
                    <Link href={`/addresses/${from}/collections/${organization.contractId}`}>
                      <a>
                        <EthAccount organization={organization} address={from} />
                      </a>
                    </Link>
                  )}
                </td>
                ...
                <td className="pr-3">
                  {to && (
                    <Link href={`/addresses/${to}/collections/${organization.contractId}`}>
                      <a>
                        <EthAccount organization={organization} address={to} />
                      </a>
                    </Link>
                  )}
                </td>
